...
static int32_t PIOS_MPU_Common_Init(void)
{
	...
	mpu_dev->gyro_reg = PIOS_Sensors_Register(PIOS_SENSOR_GYRO, (void*)mpu_dev, PIOS_MPU_gyro_callback, NULL, PIOS_SENSORS_FLAG_SEMAPHORE);
	mpu_dev->accel_reg = PIOS_Sensors_Register(PIOS_SENSOR_ACCEL, (void*)mpu_dev, PIOS_MPU_accel_callback, NULL, PIOS_SENSORS_FLAG_NONE);
	...
	if (mpu_dev->use_mag)
		mpu_dev->mag_reg = PIOS_Sensors_Register(PIOS_SENSOR_MAG, (void*)mpu_dev, PIOS_MPU_mag_callback, NULL, PIOS_SENSORS_FLAG_NONE);
#endif // PIOS_INCLUDE_MPU_MAG
